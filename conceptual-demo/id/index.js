...
function visibleIfIntersecting(entries) {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            entry.target.style.opacity = 1;
        }
        else {
            // const cur = Number(entry.target.style.opacity);
            // console.log(cur);
            // entry.target.style.opacity = cur === 1 ? 0.2 : 0;
            entry.target.style.opacity = 0.2;
        }
    });
}

const obs = new IntersectionObserver(e => visibleIfIntersecting(e), { rootMargin: '-22% 0px' });
